...
| `error` says "company/date/session could not be determined" | The message text genuinely doesn't state that field clearly enough, or uses unusual wording | Have the clerk resubmit with clearer text; if it's a recurring wording pattern, add a rule to `ParserAi.buildPrompt_()` and re-run `bun run eval` to confirm it helped |
